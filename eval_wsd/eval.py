...
def get_args(
    ...
    parser.add_argument('-con', default='', help='set the sense keys that can be considered')
...
class SensesVSM(object):
    ...
    def __init__(self, vecs_path: str, normalize=False):
        ...
        elif self.vecs_path.find('.npz') != -1:
    ...
    def match_senses(self, context_vec, lemma=None, postag=None, topn=100, constraint=None):
...
def select():
    a = os.popen('"/usr/bin/nvidia-smi" --query-gpu=memory.free --format=csv,nounits,noheader').read().strip().split(
        "\n")
    if int(a[0]) > int(a[1]):
        num = 1
    else:
...
if __name__ == '__main__':
    ...
    con_path = args.con
    if args.con != '':
        with open('three_common_keys.json', 'r') as f:
            con = set(json.load(f))
            print(len(con))
    else:
        con = None
    ...
    guess_num = 0
    # print(id2senses)
    # print(len(id2senses.keys()))
    # print(len([k for i in id2senses.values() for k in i]))
    # print(len(set([k for i in id2senses.values() for k in i])))
        ...
        for batch_idx, batch in enumerate(chunks(eval_instances, args.batch_size)):
            ...
            for sent_info in batch:
                ...
                for mw_idx, tok_idxs in idx_map_abs:
                    ...
                    elif senses_vsm.ndims == 6144:
                        currVec_c = torch.cat((currVec_c, currVec_c), dim=0)
                        currVec_c = torch.cat((currVec_c, currVec_c, currVec_c), dim=0)
                    # print("currVec_c.shape is",currVec_c.shape)

                    matches = senses_vsm.match_senses(currVec_c, lemma=curr_lemma, postag=curr_postag, topn=1,
                                                      constraint=con)
    ...
    with open('res_summary5.txt', 'a+') as f:
        f.write(str(args.sv_path) + " ")  # +"_"+str(test_num)+":"+str(guess_num)+" ")
        f.write(str(args.test_set) + " " + str(precision) + " :r: " + str(recall) + " f:" + str(f_score))
        f.write('\n')
