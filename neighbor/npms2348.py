...
def load_source_embeddings(sources: list):
    """
    ...
    for np_loader in sources:
        np_vec = np_loader['vectors']
        src = np.zeros((len(vocab), np_vec.shape[1]))
        vecs = {k: v for k, v in zip(np_loader['labels'], np_vec)}
        for sense in vocab:
...
if __name__ == '__main__':
    ...
    proj_mat2,proj_mat1 = proj_mat1,proj_mat2
    ...
    if hyper:
        ...
        alpha = torch.tensor(0.08, requires_grad=True, device=device)
    ...
    possible_emb = {'ares', 'lmms', 'lmms2348','sensem'}
    source_path = sorted(source_path)
    print('input source', source_path)

    for i in source_path:
        assert i in possible_emb, 'please choose source embedding from ares,lmms,lmms2348,sensem'
    path_dict = {
        'ares': np.load('../emb_npz/ares.npz'),
        'lmms': np.load('../emb_npz/lmms2048.npz'),
        'lmms2348': np.load('../emb_npz/lmms2348.npz'),
    ...
    dir = '/LOCAL3/robert/'
    pip_arr = [f'{dir}pip_only2348_ares_lmms2348_e7_v2.npz', f'{dir}piponly_ares_sensem_e40.npz',
               f'{dir}piponly_lmms_sensem_e11.npz']
    sup_arr = [f'{dir}sup_only2348_ares_lmms2348_e7_v2.npz', f'{dir}suponly_ares_sensem_e15.npz',
               f'{dir}suponly_lmms_sensem_e15.npz']
